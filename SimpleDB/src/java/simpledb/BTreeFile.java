...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		//2.判断该数据页是否为叶子节点，如果是则递归结束，返回该页面
		if(type == BTreePageId.LEAF){
			//使用BTreeFile.getPage()
			//It works exactly like BufferPool.getPage() , but takes an extra argument to track the list of dirty pages.
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		//3.检查原页面是否有右页面，如果有需要更新右页面和新建页面的位置关系
		...
		if (oldRightPage != null) {
			...
			//现在的位置关系是rightPage->oldRightPage，rightPage是新建的page
		}
		...
		//现在的位置关系是page->rightPage->oldRightPage，rightPage是新建的page

		//6.找出父节点，创建entry插入参数field
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//2.剪切tuples至新页面；
		//keyToUp为即将提升入父节点的中间节点
		BTreeEntry keyToUp = null;
		while(entryIterator.hasNext()){
			BTreeEntry entry = entryIterator.next();
			//Each entry stores one key and two child page ids
			//设置中间节点
			if(index == midIndex) {
			...
			if(index > midIndex) {
				...
				//每加入一个entry就更新newInternalPage的指针
		...
		//更新keyToUp的左右孩子指针，现在的位置关系是page(child page)<-keyToUp(parent entry)->newInternalPage(child page)
		parent.insertEntry(keyToUp);
		page.setParentId(parent.getId());//page(child page)<-parent(parent page)
		newInternalPage.setParentId(parent.getId());//parent(parent page)->newInternalPage(child page)
		//现在的位置关系是page(child page)<-parent(parent page)->newInternalPage(child page)
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//方向：Move from the right page to the left page
		Iterator<Tuple> tuples = rightPage.iterator();
		int numTuple = rightPage.getNumTuples();
		//1.将right page的tuples剪切到left page
		...
		//2.连接left page和right page的right sibling
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//2.将right page的entries剪切到left page
